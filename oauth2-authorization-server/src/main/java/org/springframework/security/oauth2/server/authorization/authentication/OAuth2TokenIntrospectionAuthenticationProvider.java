		...
		try {
			...
			if (StringUtils.hasText(tokenTypeHint)) {
				if (TokenType.REFRESH_TOKEN.getValue().equals(tokenTypeHint)) {
					...
					throw new IntrospectionTokenException("Unsupported token type");
				}
			}
			...
			return new OAuth2TokenIntrospectionAuthenticationToken(token, true, clientPrincipal, registeredClient.getClientId());
