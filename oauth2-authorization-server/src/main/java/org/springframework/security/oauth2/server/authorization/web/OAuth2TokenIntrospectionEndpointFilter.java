...
public class OAuth2TokenIntrospectionEndpointFilter extends OncePerRequestFilter {
			...
			OAuth2TokenIntrospectionResponse tokenIntrospectionResponse = introspectionTokenAuthentication
					.isTokenActive()
							? generateTokenIntrospectionResponse(
									introspectionTokenAuthentication.getToken(),
	...
	private OAuth2TokenIntrospectionResponse generateTokenIntrospectionResponse(AbstractOAuth2Token token,
			String clientId) {
		OAuth2TokenIntrospectionResponse.Builder builder = OAuth2TokenIntrospectionResponse.builder(true);
	...
	private static final class TokenToIntrospectionResponseFieldsMapper {
		 ...
		 * @return This builder.
		 */
		private static OAuth2TokenIntrospectionResponse.Builder extractFromJwt(final Jwt jwt,
				OAuth2TokenIntrospectionResponse.Builder builder) {
			builder.tokenType(BEARER);
			Optional.ofNullable(jwt.getSubject()).ifPresent(builder::subject);
			Optional.ofNullable(jwt.getId()).ifPresent(builder::jwtId);
			Optional.ofNullable(jwt.getAudience()).ifPresent(builder::audience);
			Optional.ofNullable(jwt.getNotBefore()).ifPresent(builder::notBefore);
			Optional.ofNullable(jwt.getClaimAsStringList(OAuth2ParameterNames2.SCOPE))
					.map(scopes -> String.join(" ", scopes)).ifPresent(builder::scope);

			Optional.ofNullable(jwt.getIssuer()).map(URL::toExternalForm).ifPresent(builder::issuer);
			return builder;
		}
